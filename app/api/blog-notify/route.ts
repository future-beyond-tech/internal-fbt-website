...
type ValidationResult =
  | { success: true; data: BlogNotifyPayload }
  | { success: false; error: string };

export async function POST(req: Request) {
  const clientIp = getClientIp(req);
  const rateLimit = checkRateLimit(
    `blog-notify:${clientIp}`,
  ...
  if (!resendApiKey || !contactEmail) {
    console.warn(
      "Blog notify email notifications are not configured. Missing RESEND_API_KEY or CONTACT_EMAIL."
    );
    return NextResponse.json(
  ...
  const { email, topic, slug } = validated.data;
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      subject: "New blog notify subscriber",
      text: `Blog notify signup
Email: ${sanitizeSingleLine(email)}
Topic: ${sanitizeSingleLine(topic ?? "general updates")}
Article: ${sanitizeSingleLine(slug ?? "unknown")}`,
    });
    ...
    console.error("Blog notify signup error:", error);
    return NextResponse.json(
      { error: "Unable to submit right now. Please try again." },
...
function validatePayload(payload: unknown): ValidationResult {
  ...
  if (topic && topic.length > 200) {
    return { success: false, error: "Topic is too long." };
  }

  if (slug && slug.length > 200) {
    return { success: false, error: "Article reference is too long." };
  }

  return {
    success: true,
    data: { email, topic, slug },
  };
}
